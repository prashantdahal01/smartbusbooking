const { City } = require("./location.model");
...
/**
 * Search locations across districts, cities, stops, and route points.
 * @param {object} params
 * @param {object} params.query
 * @returns {Promise<Array<{name: string, type: string}>>} 
 */
const searchLocations = async ({ query }) => {
  const queryText = normalizeText(query?.q);
  if (!queryText) return [];

  const limit = toPositiveInt(query?.limit, 10, 1, 20);
